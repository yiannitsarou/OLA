        ...
        if st.button("⚡ Fill & Optimize", type="primary", use_container_width=True):
            ...
            with st.spinner("🔄 Phase 2/2: Optimizing..."):
                try:
                    ...
                    # --- Auto-run Step 8 export right after Step 7/Optimization ---
                    try:
                        ...
                        # 1) Persist the optimized bytes to disk so Step 8 can read them
                        _opt_path = _root / f"OPTIMIZED_{int(_time.time())}.xlsx"
                        with open(_opt_path, "wb") as _f:
                            _f.write(output_bytes)

                        # 2) Try to locate a Step 8 module in the app folder
                        _step8_names = ["step8_fixed_final.py", "step8_final.py", "ΒΗΜΑ8.py", "vima8.py"]
                        _step8_mod = None
                        for _name in _step8_names:
                            _cand = _root / _name
                            if _cand.exists():
                                _spec = _importlib_util.spec_from_file_location(_name.replace(".py",""), _cand)
                                if _spec and _spec.loader:
                                    _step8_mod = _importlib_util.module_from_spec(_spec)
                                    _spec.loader.exec_module(_step8_mod)
                                    break

                        _step8_out = _root / "STEP8_FINAL_SCENARIO.xlsx"
                        _ran_ok = False

                        if _step8_mod is not None:
                            # Try common function signatures
                            for _fn in ["build_step8_final","run_step8","apply_step8","finalize_step8","postprocess_final"]:
                                if hasattr(_step8_mod, _fn):
                                    try:
                                        getattr(_step8_mod, _fn)(str(_opt_path), str(_step8_out))
                                        _ran_ok = True
                                        break
                                    except TypeError:
                                        # Fallback to single-arg (input only)
                                        try:
                                            getattr(_step8_mod, _fn)(str(_opt_path))
                                            # If the module wrote its own output, assume done
                                            _ran_ok = _step8_out.exists()
                                            break
                                        except Exception:
                                            pass
                                    except Exception:
                        ...
                        if not _ran_ok:
                            # 3) Fallback: self-contained Step 8 → produce consolidated FINAL_SCENARIO
                            #    by reading team sheets from optimized output and writing a
                            #    FINAL_SCENARIO sheet plus per-team sheets mirrored.
                            _wb_in = _openpyxl.load_workbook(_opt_path, data_only=True)
                            _wb_out = _openpyxl.Workbook()
                            _ws0 = _wb_out.active
                            _wb_out.remove(_ws0)

                            _frames = []
                            _cols = ['ΟΝΟΜΑ','ΦΥΛΟ','ΚΑΛΗ_ΓΝΩΣΗ_ΕΛΛΗΝΙΚΩΝ','ΕΠΙΔΟΣΗ','ΦΙΛΟΙ']

                            def _copy_sheet(_src_ws, _dst_ws, _team_name):
                                # write headers
                                for ci, h in enumerate(_cols, start=1):
                                    _dst_ws.cell(1, ci).value = h
                                _r = 2
                                # find header indices in src
                                _hdr = {}
                                for ci, cell in enumerate(_src_ws[1], start=1):
                                    if cell.value:
                                        _hdr[str(cell.value).strip()] = ci
                                for row in _src_ws.iter_rows(min_row=2):
                                    ...
                                    for h in _cols:
                                        ci = _hdr.get(h)
                                        _vals.append(str(_src_ws.cell(row=row[0].row, column=ci).value).strip() if ci else "")
                                    for ci, v in enumerate(_vals, start=1):
                                        _dst_ws.cell(_r, ci).value = v
                                    _r += 1

                            _all_rows = []
                            for _sname in _wb_in.sheetnames:
                                if _sname in ["ΒΕΛΤΙΩΜΕΝΗ_ΣΤΑΤΙΣΤΙΚΗ","ΕΦΑΡΜΟΣΜΕΝΑ_SWAPS"]:
                                    continue
                                _src_ws = _wb_in[_sname]
                                _dst_ws = _wb_out.create_sheet(_sname)
                                _copy_sheet(_src_ws, _dst_ws, _sname)
                                # also collect for FINAL_SCENARIO
                                for rr in _dst_ws.iter_rows(min_row=2, values_only=True):
                                    if not rr or not rr[0]:
                                        continue
                                    _all_rows.append((_sname,) + rr)  # prepend team

                            # FINAL_SCENARIO
                            _final = _wb_out.create_sheet("FINAL_SCENARIO")
                            headers = ['ΤΜΗΜΑ'] + _cols
                            for ci, h in enumerate(headers, start=1):
                                _final.cell(1, ci).value = h
                            for ri, row in enumerate(_all_rows, start=2):
                                for ci, v in enumerate(row, start=1):
                                    _final.cell(ri, ci).value = v

                            _wb_out.save(_step8_out)
                            _wb_out.close()
                            _wb_in.close()
                            _ran_ok = True

                        if _ran_ok and _step8_out.exists():
                            st.success("🟣 Βήμα 8: Δημιουργήθηκε το STEP8_FINAL_SCENARIO.xlsx")
                            st.download_button(
                                "⬇️ Λήψη STEP8_FINAL_SCΕΝΑΡΙΟ.xlsx",
                                data=open(_step8_out, "rb").read(),
                                file_name=_step8_out.name,
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                use_container_width=True
                            )
                        else:
                            st.info("ℹ️ Δεν εκτελέστηκε το Βήμα 8 (δεν βρέθηκε module και απέτυχε το fallback).")
                    except Exception as _e:
                        st.info("ℹ️ Το Βήμα 8 παρέλειφθη λόγω σφάλματος. Προχωρώ με το αποτέλεσμα της βελτιστοποίησης.")
